...
int villageCardTest(struct gameState *pre, struct gameState *post, int player) {
	...
	int preActionCount = pre->numActions;
	int postActionCount = post->numActions;



	if ((preHandCount == postHandCount) && preActionCount+2 == postActionCount) {
		return 0;
	}
	else {
		return 1;
...
int main() {
	...
	for (int i = 0; i < NUM_TESTS; i++) {
		...
		int cardPlay = cardEffect(village, 1, 1, 1, &post, handPos, &coinBonus);
		if (cardPlay == -1) {

		}

		// Check to see if we failed our test
		result += villageCardTest(&pre, &post, player);

	}
	printf("Tests Complete\n");
	if (result >= 1) {
		printf("Village failed %d times, check your code and try again\n", result);
	}
	else {
		printf("Village Card passed, no oddities could be recreated");
	}
	printf("--------------------------------------------------------------------------------\n");
